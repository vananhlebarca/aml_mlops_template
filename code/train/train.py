...
def main(args):
    ...
    svm_model = SVC(kernel=args.kernel, C=args.penalty, gamma='scale', probability=True).fit(data['train']['X'], data['train']['y'])
    ...
    # AUC for X_test
    #auc = roc_auc_score(data["test"]["y"], svm_model.predict_proba(data['test']['X']), multi_class='ovr')
    #print('AUC of SVM classifier on test set: {:.2f}'.format(auc))
    #run.log('auc', auc)
...
def parse_args():
    ...
    parser.add_argument('--ds', type=str, dest='dataset_arg')
